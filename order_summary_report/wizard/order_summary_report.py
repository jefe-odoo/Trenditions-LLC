    ...
    def get_lines(self):
        ...
        for obj in stock_history:
            ...
            if self.customer_ids and self.customer_reference:
                ...
                "WHERE "\
                "order_id in (select id from sale_order where state <> 'cancel' and partner_id in %s and client_order_ref LIKE %s and create_date >= %s and create_date <= %s) and "\
            ...
            if sale_obj:
                ...
                "WHERE "\
                "order_id in (select id from purchase_order where state = 'draft' and create_date >= %s and create_date <= %s) and "\
                "product_id = %s", (self.date_from,self.date_to, product.id))
